...
**包重排（agent / reminder / ledger / shared，DESIGN.md §2.2）已经合进 `main`。** 09-24 卸载重装后，按测试清单在真机上走了一遍，全部通过。清单测完就删了，要看用 `git show 8e5ee3c:TEST_PLAN.md`。清单里「main 上就没验过、顺手可以验的」那一节是可选的，下表仍按没验算。
从重排之前的版本升上来要连数据卸载，见最后一节「踩过的坑」的最后一条。

真机是荣耀 LGE-AN10，模型是火山方舟 `deepseek-v4.1-flash`。下表「真机验过」一栏里，「09-24：」后面的是重排之后那一轮补验的，其余是重排之前在 main 上验的；早先用 adb 测的句子是英文，09-24 那轮是中文。

| 功能 | 真机验过 | 还没验 |
|---|---|---|
| 调度内核 | 冒烟零漂移（`dumpsys alarm` 里有 `Alarm clock:` 段、`exactAllowReason=policy_permission`、列为 `Next wake from idle`）；「喝水」全链路漂移 259ms；锁屏全屏页会弹（肉眼确认），点「完成」无残留排期。09-24：重启手机后闹钟还在 | **48 小时放置测试**（DESIGN §9.3，唯一还缺的硬证据） |
| 对话 agent | 流式逐字、工具参数边流边出；骨架阶段点「停」几百毫秒内撤回；「hello」只回话；重装后对话还在。09-24：打开不闪（杀进程重开直接是记录、停在最后一条） | 断网 / 坏 key 的回退；推理模型的思考块（这家供应商不发）；思考开关网关收不收；「↓ 新内容」 |
| Markdown、画图 | 09-24：标题、列表、表格、代码块、链接，流式时不露 `**`，「#12 那笔」不变标题；问「画个图」出结论 + 图，点开缩放，深色换色，坏图退回原文，「停」撤掉整轮 | —— |
| 问卡与痕 | 直接建提醒只留痕、闹钟排上；一题 / 两题问卡点选 +「其他…」→ 建成；历史重启后按新样子画回来；痕点进编辑页。09-24：编辑页的「依据」；速记交给对话页；改多笔的痕「改了 N 笔 ⌄」 | 不点直接说；问卡等着时点「停」、杀进程后读回「没答」；多笔的对账问卡；深色 |
| 当天事项 | 收尾时刻准点响（漂移 23ms）、不弹全屏、顺延到第二天、通知上「完成」、两条合成一组、改收尾时刻全部跟着挪。09-24：中文句子 | 列表的「拖 N 天」；小组件上的当天事项；重复的当天事项 |
| 提醒列表（时间轴） | 排版；点圈完成 → 闹钟消失 → 撤销 → 闹钟回来；左滑删除无残留。09-24：每一步之后小组件跟着变；重复规则的人话 | 告警态（过点没响、没排上）；深色；重复提醒的完成 / 撤销 |
| 设置页、编辑页 | 排版、漂移写人话；编辑页新建 → 滚轮 → 存 → 闹钟；页内删除 → 闹钟消失。09-24：体检从全缺开到全绿，「去开」回来自己变对勾 | 深色；「当天之内」切换；CUSTOM 规则保留 |
| 模型服务（印章、纸签、配置页） | 09-24：「测一下」；纸签上网关只写主机名；飞行模式下出错处有「手动填一条」 | 印章三态；改完 key 下一句就用新的；飞行模式下变墨灰再盖回朱砂 |
| 桌面小组件、速记 | 3×2 两条的版式；纸从小组件长出来（和截图差 4px 以内）；本地语音（`dumpsys audio` 里是 `VOICE_RECOGNITION`、没有 YOYO，模型首次加载 1.9s）。09-24：抬头 / 某条 / 空白三个去处；深链（app 开着、关着） | 4×2 / 4×3 / 4×4（只能手动拖） |
| 记账 | 样本导入 → 整理（15 秒 20 条）→ 抽屉 → 三层 → 一笔「去对话里说」→ 对账一轮；21:30 闹钟在 `dumpsys alarm` 里。09-24：真付一笔 → 实时进库 → 手动整理；21:30 通知点「现在」；转给别人算支出、不作废 | 3 小时后自动整理；21:30 通知的「晚点」「今天算了」 |
| 抓取页 | 09-24：实时收到当场显示；「现在抓一下」；展开看全文和晚了几秒；关掉使用权变红、去开回来连上；**「重连」在荣耀上管用** | —— |
