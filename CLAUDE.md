...
- **打开 app 先闪一下空对话**（2026-09-24 已修，没编译、没上真机）：对话记录是异步从 `chat.db` 读的，读完之前列表是空的，
  对话页就先画了招呼语和例句、再淡出换成聊天记录；列表出来时还停在最上面、下一帧才被拉到底。
  现在 `ChatViewModel.restored` 没变 true 之前对话页中间空着，`MainActivity` 按住第一帧让系统开屏一直盖着（最多 1 秒），列表一出来就停在最后一条。见 DESIGN.md §6.8「对话落盘」。
