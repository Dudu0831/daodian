...
- **授权条钉在输入框上方**（`ui/chat/ApprovalDock.kt`，设计稿：<https://claude.ai/artifact/5Lu9cTegZ6wC5LYViWhtmJ>），
  像 Claude 那样：聊天里的卡片只是虚线草稿「等你确认」，点头在授权条上 —— 卡片滚出屏幕了，授权条还在手边。
  写成人话（「要建一条提醒 · 交房租 · 9月20日 周六 15:00 · 不重复」），「好」实心、「不」窄的描边。
  - 点「好」工具才执行、卡片落印；点「不」卡片退成「没记下」、原本要建的那件事划掉摆着，模型收到「用户没同意」后自己说一句。
  - **授权条亮着时输入框照样能打字，发出去 = 「不」+ 改口**（`Approval.Redirected`）：这一轮就此收住（不再调模型），
    那句话作为新的一句发出去，模型在新一轮带着上文重办。打字时授权条缩成一行、按钮变灰，输入框描朱砂边。
  - 桌面速记没有输入框，只有「好 / 不」；点了「不」模型回一句就完，不再自动接着听。
  - 只有「好 / 不」，没有「以后都允许」—— 那等于绕开「写操作要问」这条规矩。
