...
`ChatMessage` 里助手侧只有一个 `AssistantTurn`：思考 → 工具行 → 正文 → 卡片，
四块摞在一条消息上，从流式态一路长成终态，中途不换消息类型。上一版是流式占位消失、
卡片另起一条，模型说的那段话被丢掉了 —— 而那段话往往是唯一解释「它为什么这么排」的东西。

三条摆放规矩：

1. **工具行在正文上面**。模型的动作先于它的解说，「这一回合动了手」不该要读完整段话才知道。
2. **工具行只露工具名，参数不上屏**。参数是给日志看的。跑着的时候整行呼吸式明暗
   （那几秒里它是屏幕上唯一在动的东西），落定换成朱砂对勾、字压到 muted。
3. **有卡片就藏工具行，没卡片就留**。卡片上的「已记下」和工具名说的是同一件事，
   留着是同义反复；但工具跑了却没长出卡片（校验闸门拦下了算错的时间，见 §6.5），
   它就是「模型动过手」的唯一痕迹。判定见 `AssistantTurn.showToolRow`。
