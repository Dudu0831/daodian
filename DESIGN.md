...
|---|---|
| 抬头 | 提醒列表 |
| 整块空白 / 右下角墨印 | 桌面速记，一进来就开始听（§8.3）。墨印自己不挂点击，点它落到整块上 —— 这样桌面给的 `sourceBounds` 才是整块小组件的框。对话页从 app 图标进 |
| 下一条 / 某一行 | 编辑那一条 |
| 完成圈 / 行尾 `✓` | 直接完成 |
...
**点击挂在整块小组件上**，墨印自己不挂：点墨印会落到整块上，桌面随点击给的 `Intent.sourceBounds` 就是整块小组件的框，墨印的位置从右下角倒推（内边距 10dp、直径 44dp）。试过只让墨印可点、再拿 `OPTION_APPWIDGET_*` 报的尺寸去拼整块的框 —— 荣耀报的宽是 224 / 302dp，实际 242dp，哪个都对不上，第一帧就错位。代价：点小组件的空白处也是「说一句」，不再进对话页（对话页从 app 图标进）。拿 `sourceBounds` 要求这个 PendingIntent 是 `FLAG_MUTABLE` —— IMMUTABLE 会把桌面的 fill-in 整个丢掉，这也是真机上踩出来的。（更早还试过从墨印的小圆长出来，看着像一颗按钮炸开，怪。）
